...
/* 	  --for i = 1 to MAX_NUMBER_OF_PROCESSES
      for i = 1 to 1
        th = alloc Thread
		th.Init ("TestStarvation_GreedyProcess")
        th.Fork (TestStarvation_GreedyProcess, i)
      endFor */
	  
	  --for i = 1 to MAX_NUMBER_OF_PROCESSES
	  --for i = 2 to MAX_NUMBER_OF_PROCESSES
      for i = 1 to MAX_NUMBER_OF_PROCESSES
        th = alloc Thread
		th.Init ("TestStarvation_SmallProcess")
        th.Fork (TestStarvation_SmallProcess, i)
      endFor
...
-----------------------------  TestStarvation_GreedyProcess  ---------------------------------
-- 
-- Wrapper for launching TestStarvation with a process requirement a lot of memory
-- We have to call this wrapper because fork method of the Thread class allows
-- only parameters for thread ID and one integer parameter. 
--
  function TestStarvation_GreedyProcess (myID: int)

	  --TestStarvation (myID: int, num_iters: int, max_pages: int)
      --TestStarvation (myID, NUMBER_ITERATIONS_2, NUMBER_OF_PHYSICAL_PAGE_FRAMES)
	  TestStarvation (myID, NUMBER_ITERATIONS_2, MAX_PAGES_PER_VIRT_SPACE)
  
    endFunction

-----------------------------  TestStarvation_SmallProcess  ---------------------------------
-- 
-- Wrapper for launching TestStarvation with a process requirement a lot of memory
-- We have to call this wrapper because fork method of the Thread class allows
-- only parameters for thread ID and one integer parameter. 
--
  function TestStarvation_SmallProcess (myID: int)
  
	  --TestStarvation (myID: int, num_iters: int, max_pages: int)
	  --TestStarvation (myID, NUMBER_ITERATIONS_2, MAX_PAGES_PER_VIRT_SPACE)
	  TestStarvation (myID, NUMBER_ITERATIONS_2, 1)
	  
    endFunction

-----------------------------  TestStarvation  ---------------------------------
--
-- This function is used to test starvation.  
-- It was copied from TestFrameManager but with some constants now parameratized 
--
-- NUMBER_ITERATIONS_2 is now parameratized as num_iters
-- MAX_PAGES_PER_VIRT_SPACE is now parameratied as num_pages
--
  function TestStarvation(myID: int, num_iters: int, max_pages: int)
	  ...
	  if (myID == 5)
		max_pages = MAX_PAGES_PER_VIRT_SPACE
      else
		max_pages = 1
	  endIf
      for i = 1 to num_iters    	-- Parameratized now
        for sz = 1 to max_pages-1	-- Parameratized now
...
-- NUMBER_OF_PHYSICAL_PAGE_FRAMES = 27  
