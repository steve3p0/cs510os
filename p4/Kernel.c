  ...
  behavior ThreadManager
      ...
      method Init ()
		...
		var i: int		
		  ...
		  -- This code should really be in the while loop!!!!
		  -- Can't cast an int to a char array.
		  -- How important is it that these threads have unique names?
		  -- Update: very helpful to have unique thread names when debugging
		  ...
		  -- Initialize thread table and list of free threads
          for i = 0 to MAX_NUMBER_OF_PROCESSES-1
			-- We can't cast an int to a string
			-- So either we initialize w/o unique names
			-- or our code looks like the code above.
      ...
      method GetANewThread () returns ptr to Thread
        ...
        -- 
		  ...
		  -- If there are no free threads - wait for one.
		  -- Hoare Semantics
		  -- if freeList.IsEmpty()
		  --   aThreadBecameFree.Wait(&threadManagerLock)
		  -- endIf
		  
		  -- Use MESA semantics to wait for a thread to free up
		  while freeList.IsEmpty()
			aThreadBecameFree.Wait(&threadManagerLock)
		  endWhile
		  
		  -- A thread on the free list became available
		  -- Let's remove it and use it as our new thread
		  -- BUT... let's first check and make sure freeList isn't empty
		  if freeList.IsEmpty ()
			FatalError("A list of free threads 'freeList' is empty when it should have at least one free thread")
		  endIf
		  
		  t = freeList.Remove()
		  -- Okay maybe this is overkill -- but it doesn't hurt
		  if (t == null)
	        FatalError("Attempt to remove a thread from an empty list")
		  endIf
      ...
      method FreeThread (th: ptr to Thread)
        ...
        -- 	
		  ...
		  -- Let's free up a thread!
		  th.status = UNUSED
		  freeList.AddToEnd(th)
		  
          -- Signal that a thread is free
  ...
  behavior ProcessManager
      ...
      method Init ()
		...
		var i: int		
		  ...
		  -- Initialize list of available process control blocks
          for i = 0 to MAX_NUMBER_OF_PROCESSES-1
			processTable[i].Init ()
      ...
      method GetANewProcess () returns ptr to ProcessControlBlock
        ...
        --         
		  ...
		  --print ("ProcessManager.GetANewProcess\n   ")
		  p = null
		  
		  -- ENTERING CRITICAL SECTION
		  processManagerLock.Lock()
		  
		  -- If there are no free processes - wait for one.
		  -- Hoare Semantics
		  -- if freeList.IsEmpty()
		  --   aProcessBecameFree.Wait(&processManagerLock)
		  -- endIf
		  
		  -- Use MESA semantics to wait for a process to free up
		  while freeList.IsEmpty()
			aProcessBecameFree.Wait(&processManagerLock)
		  endWhile
		  
		  -- A process on the free list became available
		  -- Let's remove it and use it as our new process
		  -- BUT... let's first check and make sure freeList isn't empty
		  if freeList.IsEmpty ()
			FatalError("A list of free process 'freeList' is empty when it should have at least one free process")
		  endIf
		   
		  p = freeList.Remove()
		  -- Okay maybe this is overkill -- but it doesn't hurt
		  if (p == null)
	        FatalError("Attempt to remove a process from an empty list")
		  endIf
		  
		  -- increment nextPid
		  nextPid = nextPid + 1
		  
		  p.pid = nextPid         -- The process ID
		  p.status = ACTIVE       -- ACTIVE, ZOMBIE, or FREE
      ...
      method FreeProcess (p: ptr to ProcessControlBlock)
        ...
        -- to the FREE list.
		  ...
		  -- Let's free up a process!
		  p.status = FREE
		  freeList.AddToEnd(p)
		  
          -- Signal that a process is free
...
-----------------------------  FrameManager  ---------------------------------
  ...
  behavior FrameManager
      ...
      method GetNewFrames (aPageTable: ptr to AddrSpace, numFramesNeeded: int)
	    -- 
	    -- This routine handles requests for frames. 
		-- It waits until there are enough free frames available and then allocates 
		-- stores the address them in the AddresSpace object passed to this method.
	    --
	  	  var bitMapIndex: int
			frameAddr: int
			i: int
		  ...
		  -- (2) Wait on newFramesAvailable until there are enough free
		  --     frames to satify the request
		  while numberFreeFrames < numFramesNeeded
			newFramesAvailable.Wait (&frameManagerLock)
		  endWhile
			
		  -- (3) Do a loop for each the frames
		  for i = 0 to numFramesNeeded - 1
		    -- (a) determine which frame is free
		    --  (find and set a bit in the framesInUse BitMap)
			bitMapIndex = framesInUse.FindZeroAndSet()

			-- (b) figure out the address of the free frame
			-- frameAddr = self.GetAFrame()
			-- or --
			-- frameAddr = aPageTable.ExtractFrameAddr(bitMapIndex)
			-- Compute and return the physical address of the frame...
			-- When in doubt - Do as GetAFrame does.
			frameAddr = PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME + (bitMapIndex * PAGE_SIZE)
      ...
      method ReturnAllFrames (aPageTable: ptr to AddrSpace)
		-- 
		-- This routine will free up all frames with addresses in AddrSpace
		-- It's purpose is needed to free up memory held by a thread that 
		...
		-- 
		  ...
		  -- Look at aPageTable.numberOfPages to see how many are returned.
		  numFramesReturned = aPageTable.numberOfPages
		  
		  -- Iterate page table to see which frames this address space is using		  
		  for i = 0 to numFramesReturned - 1
		    -- Get the address of the frame at index i
		    frameAddr = aPageTable.ExtractFrameAddr (i)
			
			-- Get the bit number from the frame address
			-- How do we do this?  Do the reverse of what GetNewFrames does:
			bitMapIndex = (frameAddr - PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME) / PAGE_SIZE
			framesInUse.ClearBit(bitMapIndex)
		  endFor
		  
		  -- Adjust the number of free frames
		  numberFreeFrames = numberFreeFrames + numFramesReturned
		  
		  -- Notify waiting threads that more frames are available
		  newFramesAvailable.Broadcast (&frameManagerLock)
		  
		  -- TODO: possibility of starvation: read further into instructions
