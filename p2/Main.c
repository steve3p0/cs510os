...
----------------------------- Steve's tests -------------------------------- -
  function StevesTests()
	  var
		  t : ptr to Thread

	  print("use a null pointer here and see what happens..\n")
	  t.status = READY
    endFunction
...
-----------------------------  ProducerConsumer  ---------------------------------
  ...
  var
	...
	-- Semaphore to count number of items in the buffer
	fillCount: Semaphore = new Semaphore
	-- Semaphore to count available spaces in the buffer
	emptyCount: Semaphore = new Semaphore
	-- Mutex to make sure producers don't write to same slot in the buffer
	buffer_mutex: Mutex = new Mutex

  function ProducerConsumer ()
	  -- Initialize the fill count to zero, cuz we haven't produced anything yet
	  fillCount.Init(0)
	  -- Initilize the empty count to the buffer size, cuz all the slots are empty
	  emptyCount.Init(BUFFER_SIZE)
	  -- Initilize the buffer mutex lock
	  buffer_mutex.Init()
  ...
  function Producer (myId: int)
      ...
      for i = 1 to 5
		...
		-- Decrement empty count and wait for semaphore
        emptyCount.Down ()
		-- Lock the buffer mutex to prevent producers writing to same slot
		buffer_mutex.Lock ()
		...
		-- Unlock buffer for next producer to write to buffer
		buffer_mutex.Unlock ()
		-- Increase fill count to possibly wake up consumers
		fillCount.Up()
  ...
  function Consumer (myId: int)
      ...
      while true
        -- Perform synchroniztion...
		-- Decrement fillcount and wait for semaphore
		fillCount.Down ()
		-- Lock buffer to prevent consumers consuming same slot 
		buffer_mutex.Lock ()
        ...
        -- Perform synchronization...
		-- Unlock buffer for next consumers to consume
		buffer_mutex.Unlock ()
		-- Increment empty count so that producers know whether buffer 
		-- is full or not
		emptyCount.Up ()
