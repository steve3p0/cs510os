    ...
    --         on the mutex.
		...
		method Lock ()
			...
			elseIf heldBy
              ...
              -- Add the current thread to sleep and send it to the FRONT 
			  -- of the waiting list, it has been waiting already, right?
              -- waitingThreads.AddToFront(currentThread)
              waitingThreads.AddToEnd(currentThread)
              currentThread.Sleep()
			else
			  -- print("___ Locked Request Granted to current thread: ")
