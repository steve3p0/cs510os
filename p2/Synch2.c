  ...
  behavior Mutex
    ...
    --         on the mutex.
		...
		method Lock ()
			...
			-- Save the Interrupt state and disable it 
			oldIntStat = SetInterruptsTo(DISABLED)

			-- Entering critical section 
			if self.IsHeldByCurrentThread()
              -- print (heldBy.name)
              -- print ("%s already has lock '\n", heldBy.name)
			  FatalError ("Lock Request Failed: Current thread already has lock")
			
			elseIf heldBy
              -- Lock is held by another thread
              -- Add the current thread to sleep and send it back 
              -- to the FRONT of the waiting list, it has been waiting already, right?
              -- waitingThreads.AddToFront(currentThread)
              waitingThreads.AddToEnd(currentThread)
              currentThread.Sleep()
			else
              -- print("___ Lock is availble and is requested by: ")
              -- print (currentThread.name)
              -- print("\n")

              heldBy = currentThread

              -- print("\tLocked Request Granted to current thread: ")
              -- print (heldBy.name)
              -- print("\n\n")
            endIf
            ...
            oldIntStat = SetInterruptsTo(oldIntStat)
            -- Interrupt state is now restored
        endMethod
		...
		method Unlock()
            ...
            -- Entering critical section
            if !heldBy
              -- print("____ Current thread: \n")
              -- print(currentThread.name)
              -- print(" requested unlock but no lock exists \n")
              -- FatalError("Unlock Request Failed: mutex is not locked")

            elseIf self.IsHeldByCurrentThread()
               -- print("____ Current thread: ")
               -- print(currentThread.name)
               -- print(" is requesting to unlock mutex.\n")

               -- Set heldBy to null because it is FREE NOW!!!
               heldBy = null
	
               -- Get a waitning thread, if one exists.
               t = waitingThreads.Remove()
			
               -- Check to make sure there is a waiting thread
               if t
                    t.status = READY
                    readyList.AddToEnd(t)
                    heldBy = t
               endIf
                ...
                -- print("____ Current thread: \n")
                -- print(currentThread.name)
                -- print(" requested unlock but does not have lock.\n")
                -- print(heldBy.name)
                -- print(" has the lock'\n")
                FatalError("Unlock Request Failed")
            endIf
            -- Exiting critical section
